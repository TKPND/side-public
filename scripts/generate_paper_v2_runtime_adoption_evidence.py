#!/usr/bin/env python3
"""Generate replayable risk_contract.v2 paper runtime adoption evidence."""
...
SCHEMA_VERSION = "risk_contract_v2_paper_runtime_adoption_evidence.v1"
BOUNDARY = "risk_contract_v2_paper_runtime_adoption_evidence"
DEFAULT_REPORT_DIR = Path("reports/risk-contract-v2/paper-runtime-adoption")
JSON_REPORT_NAME = "paper_v2_runtime_adoption_evidence.json"
MD_REPORT_NAME = "paper_v2_runtime_adoption_evidence.md"
...
SOURCE_EVIDENCE_PATHS: tuple[str, ...] = (
    ...
    "rust/side-cli/examples/paper_v2_evidence_replay.rs",
...
REPLAY_CONTRACT: tuple[dict[str, str], ...] = (
    {
        ...
        "claim": "Every replayed paper evidence payload exposes risk_contract.v2 and validator-result v2 proof.",
        "evidence": "paper evidence fields risk_contract_schema_version, risk_contract_version, validator_result_schema_version, and validated_schema_ref.",
        "status": "required",
    },
    {
        "id": "v2_validator_replay",
        "claim": "Every emitted paper v2 decision artifact revalidates through scripts/validate_risk_contract.py.",
        "evidence": "validator replay payload has risk_contract_validator_result.v2 and valid=true.",
        "status": "required",
    },
    {
        "id": "paper_cap_runtime_application",
        "claim": "The cap decision applies paper slot-allocation runtime sizing in apply mode.",
        "evidence": "cap paper evidence and artifact show unit_paper_slot_allocation, runtime_sizing_applied=true, and metrics_rescaled=false.",
        "status": "required",
    },
    {
        "id": "paper_size_continue",
        "claim": "The size decision continues without paper runtime sizing override.",
        "evidence": "size paper evidence has continued execution, runtime_sizing_applied=false, and no runtime_size_overrides.",
        "status": "required",
    },
    {
        "id": "paper_reject_stop",
        "claim": "The reject decision stops before paper tick mutation.",
        "evidence": "reject paper evidence is stopped, position_mutation=false, and helper output has should_run_tick=false.",
        "status": "required",
    },
    {
...
def build_protected_surface_guard(report_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "reason": "fresh risk_contract.v2 paper replay outputs only; protected historical/report/contract roots are not write targets",
    }
...
def policy_for(decision_class: str) -> dict[str, Any]:
    rule: dict[str, Any] = {
        "id": f"phase-v2-paper-evidence.{decision_class}",
    ...
    return {
        "version": "risk-policy.v1.v2-paper-evidence-test",
        "owner": "side-risk-contract-v2-paper-runtime-adoption",
...
def run_paper_v2_replay(run_name: str, decision_class: str, report_dir: Path) -> dict[str, Any]:
    ...
    if result.returncode == 0:
        ...
        evidence_paths = stdout.get("evidence_paths")
        if not isinstance(evidence_paths, list) or len(evidence_paths) != 1:
            raise ValueError(f"{run_name} must emit exactly one paper evidence path")
        paper_evidence_path = repo_path(Path(evidence_paths[0]))
        paper_evidence = load_json(paper_evidence_path)
        candidate_id = paper_evidence["candidate_id"]
        candidate_path = repo_path(Path(paper_evidence["candidate_artifact_path"]))
        artifact_path = repo_path(Path(paper_evidence["decision_artifact_path"]))
    ...
    row = {
        ...
        "paper_evidence_path": display_path(paper_evidence_path) if paper_evidence_path else None,
...
def run_paper_v2_replays(report_dir: Path) -> list[dict[str, Any]]:
    assert_allowed_report_dir(report_dir)
    return [
        run_paper_v2_replay(spec["run_name"], spec["decision_class"], report_dir)
...
def replay_row_passed(row: dict[str, Any]) -> bool:
    ...
    return (
        ...
        and row.get("stdout", {}).get("contract_version") == "v2"
        and paper_evidence.get("risk_contract_schema_version") == "risk_contract.v2"
        and paper_evidence.get("risk_contract_version") == "v2"
        and paper_evidence.get("validator_result_schema_version") == "risk_contract_validator_result.v2"
        ...
        version_proofs.append(
            {
                ...
                "risk_contract_schema_version": paper_evidence.get("risk_contract_schema_version"),
                "risk_contract_version": paper_evidence.get("risk_contract_version"),
                "validator_result_schema_version": paper_evidence.get("validator_result_schema_version"),
                "validated_schema_ref": paper_evidence.get("validated_schema_ref"),
                "validator": paper_evidence.get("validator"),
            }
        )
    passed = len(version_proofs) == len(RUN_SPECS) and all(
        proof["risk_contract_schema_version"] == "risk_contract.v2"
        and proof["risk_contract_version"] == "v2"
        ...
        and proof["validator"] == "scripts/validate_risk_contract.py"
...
def cap_runtime_application_check(replay_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    paper_evidence = cap["paper_evidence"]
    application = cap["artifact"]["application"]
    stdout = cap["stdout"]
    effective_size_equals_allowed_size = (
        paper_evidence.get("actual_effective_size")
        == paper_evidence.get("allowed_size")
        == application.get("effective_size")
    )
    overrides = stdout.get("runtime_size_overrides", [])
    passed = all(
        [
            cap.get("passed") is True,
            paper_evidence.get("decision_class") == "cap",
            paper_evidence.get("execution_state") == "continued",
            paper_evidence.get("risk_mode") == "apply",
            paper_evidence.get("requested_size_basis") == "unit_paper_slot_allocation",
            paper_evidence.get("runtime_sizing_applied") is True,
            paper_evidence.get("cap_application_status") == "applied",
            ...
            effective_size_equals_allowed_size,
            stdout.get("should_run_tick") is True,
    ...
    return {
        "passed": passed,
        "source": cap.get("paper_evidence_path"),
        "application_status": application.get("application_status"),
        "runtime_sizing_applied": paper_evidence.get("runtime_sizing_applied"),
        "requested_size_basis": paper_evidence.get("requested_size_basis"),
        "allowed_size": paper_evidence.get("allowed_size"),
        "actual_effective_size": paper_evidence.get("actual_effective_size"),
        "effective_size_equals_allowed_size": effective_size_equals_allowed_size,
        "metrics_rescaled": application.get("metrics_rescaled"),
        "should_run_tick": stdout.get("should_run_tick"),
        "runtime_size_override_count": len(overrides) if isinstance(overrides, list) else None,
    }


def size_continue_replay_check(replay_rows: list[dict[str, Any]]) -> dict[str, Any]:
    size = rows_by_name(replay_rows)["size"]
    paper_evidence = size["paper_evidence"]
    ...
    passed = all(
        [
            ...
            paper_evidence.get("decision_class") == "size",
            paper_evidence.get("execution_state") == "continued",
            paper_evidence.get("runtime_sizing_applied") is False,
    ...
    return {
        "passed": passed,
        "source": size.get("paper_evidence_path"),
        "execution_state": paper_evidence.get("execution_state"),
        "runtime_sizing_applied": paper_evidence.get("runtime_sizing_applied"),
        "application_status": application.get("application_status"),
        "should_run_tick": stdout.get("should_run_tick"),
        "runtime_size_override_count": len(overrides) if isinstance(overrides, list) else None,
...
def reject_stop_replay_check(replay_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    paper_evidence = reject["paper_evidence"]
    application = reject["artifact"]["application"]
    stdout = reject["stdout"]
    passed = all(
        [
            reject.get("passed") is True,
            paper_evidence.get("decision_class") == "reject",
            paper_evidence.get("execution_state") == "stopped",
            paper_evidence.get("position_mutation") is False,
            paper_evidence.get("runtime_sizing_applied") is False,
            application.get("execution_state") == "stopped",
            application.get("runtime_sizing_applied") is False,
            stdout.get("should_run_tick") is False,
            stdout.get("runtime_size_overrides") == [],
        ]
    )
    return {
        "passed": passed,
        "source": reject.get("paper_evidence_path"),
        "execution_state": paper_evidence.get("execution_state"),
        "position_mutation": paper_evidence.get("position_mutation"),
        "runtime_sizing_applied": paper_evidence.get("runtime_sizing_applied"),
        "application_status": application.get("application_status"),
        "should_run_tick": stdout.get("should_run_tick"),
        "runtime_size_override_count": len(stdout.get("runtime_size_overrides", [])),
    }
...
def compact_run(row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "paper_evidence_path": row.get("paper_evidence_path"),
        "artifact_path": row.get("artifact_path"),
        "execution_state": paper_evidence.get("execution_state"),
        "contract_version": paper_evidence.get("risk_contract_version"),
        "validator_result_schema_version": paper_evidence.get("validator_result_schema_version"),
        "should_run_tick": stdout.get("should_run_tick"),
        "runtime_size_override_count": len(stdout.get("runtime_size_overrides", []))
        if isinstance(stdout.get("runtime_size_overrides"), list)
        else None,
...
def build_paper_v2_runtime_adoption_evidence(
    *,
    ...
    rows = replay_rows if replay_rows is not None else run_paper_v2_replays(report_dir)
    checks = {
    ...
    return {
        ...
        "summary": {
            ...
            "diff_base": diff_base,
            "implementation_scope": "paper_v2_evidence_replay_only",
        },
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# risk_contract.v2 Paper Runtime Adoption Evidence",
        "",
    ...
    lines.extend(
        [
            ...
            "## Run Manifest",
            "",
            "| Run | Decision | Return | Execution | Contract | Validator | Should Run Tick | Overrides | Artifact | Passed |",
            "|---|---|---:|---|---|---|---:|---:|---|---:|",
        ]
    )
        ...
        lines.append(
            ...
            + " | ".join(
                [
                    ...
                    markdown_cell(run["should_run_tick"]),
                    markdown_cell(run["runtime_size_override_count"]),
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        report = build_paper_v2_runtime_adoption_evidence(
        ...
        print(
            f"generate_paper_v2_runtime_adoption_evidence.py: {exc}",
            file=sys.stderr,
        )
        return 1
