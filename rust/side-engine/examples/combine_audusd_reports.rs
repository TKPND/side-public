//! combine_audusd_reports — Phase 52 Plan 02 one-off script
//!
//! Reads FOMC and NFP individual report.json files, combines them into a
//! CombinedEventReport (ECB = empty Vec), writes:
//!   - docs/reports/v4.2-audusd/report.json
...
fn main() -> anyhow::Result<()> {
    let fomc_json = fs::read_to_string("../docs/reports/v4.2-audusd/fomc/report.json")?;
    let nfp_json = fs::read_to_string("../docs/reports/v4.2-audusd/nfp/report.json")?;
    ...
    fs::create_dir_all("../docs/reports/v4.2-audusd")?;

    // Write combined report.json
    let json = serde_json::to_string_pretty(&combined)?;
    fs::write("../docs/reports/v4.2-audusd/report.json", &json)?;
    ...
    fs::write("../docs/reports/v4.2-audusd/report.md", &md)?;
    ...
    println!("Done: docs/reports/v4.2-audusd/report.{{json,md}}");
    Ok(())
}
